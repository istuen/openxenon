...
- **状态**: Accepted（Promote from `.openxenon/drafts/design-asset-convergence-v070.md`）
- **创建日期**: 2026-08-09
...
- **关联**:
  ...
  - `RFC-0013-versioning-policy.md`（版本号中性原则）
  - `RFC-0019-draft-promote-routing.md`（router + L2 分层锁定）
  - `RFC-0025-stack-operation-followup.md`（D2.4 git-stack 按需引用锁定）
...
6. **D5**：8 条错误处理 ForbiddenConstructs 归 `oxn-engine-domain` 的错误契约段
...
### PR-G：Critical Name Collision 修复
...
**引用 cascade（~6 文件）**：
- `packages/engine/src/oxl/scope/oxn-scope.ts` — Scope type 保留 `ReferenceScope` 为 export alias
...
**引用 cascade**：
- `promote-target-aware-workflow.md:41/42/97/113` 删 `draft-skeleton-fork` 引用，改 `asset-create`
...
### PR-I：AssetMap stub 标注 + bug-fix-blueprint 接入
...
5. `Roadmap/suggest.ts` 关键词消歧：`test` 词在 scene-test 优先
...
| # | 风险 | 缓解措施 |
|---|---|---|
| R1 | D1 合并 draft domain 后，旧 promote-only 引用需要全 cascade | sync-domain-glossary + skill locales 同步更新 |
| R2 | D7 Scope → ReferenceScope 重命名涉及 6 文件 cascade，旧名 export 保留为 alias 防破坏 | v0.7 deprecated alias + v0.8 移除 |
| R3 | PR-H 6 个 Workflow 删除涉及 builtin skeleton 与 skill locales | skill locales 同步更新 |
