...
详见 ADR-0054 三边界框架 + ADR-0055 Blueprint 组合模板。
...
> **本节描述的 `type "task"|"plan"|"explore"` 字段已被废弃**。v0.6.1-alpha.0 P0 移除 builtin blueprint 模板的 `type` 字段，v0.6.1-alpha.4 三边界 RFC 正式标记 Superseded（ADR-0052）。
>
> **替代方案**：
...
详见 ADR-0056 External inline + 状态管理 + ADR-0053 Superseded ADR-0048（library/external Asset 类型收敛的史料）。
