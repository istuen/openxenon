...
| **L2** | Domain | 业务语义自治 | Arsenal（资产生命周期，v0.0.x 兼容）+ **Domain**（DDD 限界上下文）+ **Work**（编排器） |
...
| L2 Domain | `src/arsenals/`, `.openxenon/domains/`, `.openxenon/blueprints/`, `.openxenon/works/` |
...
| **L2 依赖 L1** | Domain / Work 不得直接操作文件，必须通过 L1 Port |
| **L3 可独立演进** | CLI / Skill / Hall 可替换实现，调用接口稳定即可 |

## 7. 下一章

- [Domain 详解](./domain.md) — 业务限界上下文
- [Blueprint 详解](./blueprint.md) — 技术流水线模板
- [Work + Task 详解](./work-and-task.md) — 编排与执行
