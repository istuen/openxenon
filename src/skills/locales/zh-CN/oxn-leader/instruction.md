...
> **与 `/oxn-work` 的关系**：本 skill 教 AI 怎么 **驱动** 状态机（run/submit/status）。`/oxn-work` 教 AI 怎么 **创建** work（`work new`）。两者互补，**不是替代**。

## 前置条件
- 必须在 OXN 项目根目录下执行（已运行 `oxn init`）
- 必须有 Blueprint（`oxn blueprint new <name>` 创建，或手写在 `.openxenon/blueprints/<name>.oxn`）
