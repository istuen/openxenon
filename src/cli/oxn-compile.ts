  ...
  try {
    ...
    if (doc.parseResult?.parserErrors) {
      for (const err of doc.parseResult.parserErrors) {
        diagnostics.push({
          severity: 'error',
          line: err.range?.start?.line ?? err.line ?? 0,
          column: err.range?.start?.character ?? err.column ?? 0,
          message: err.message,
          code: 'OXN_002'
        })
      }
...
export default defineCommand({
  ...
  async run(ctx) {
    ...
    if (sourcePath.endsWith('.oxn')) {
      ...
      if (!syntaxCheck.ok) {
        const lines = syntaxCheck.diagnostics.map(d =>
          `  ${d.severity === 'error' ? '✗' : '⚠'} [${d.code}] L${d.line}:${d.column} ${d.message}`
        ).join('\n')
        return outputError({
          code: 'OXN_COMPILE_FAILED',
          message: `语法校验失败:\n${lines}`
