  ...
  async run(ctx) {
    ...
    try {
      ...
      if (assetType === 'blueprint') {
        ...
        const assetDir = isGlobal
          ? join(process.env.HOME || '~', '.openxenon', 'forges', 'blueprints', blueprintName)
          : join(boundaryPath, 'forges', 'blueprints', blueprintName)

        mkdirSync(assetDir, { recursive: true })

        const canonicalBlueprint = {
          name: blueprintName,
          stages: frozen.stages?.map((s: { id: string; name: string; target?: any; action?: any; probes?: any; deps?: string[] }) => ({
            id: s.id || s.name,
            name: s.name || s.id,
            target: s.target,
            action: s.action,
