...
import {
  OxnBuiltinRegistry,
  getBuiltinRegistry,
} from "../scope/oxn-builtin-registry";
import {
  OxnWorkspaceManager,
  createWorkspaceManager,
  getWorkspaceManager,
} from "../scope/oxn-workspace-manager";
...
describe("compareProps — 跨 AST Props 比对", () => {
  test("完全匹配通过", () => {
    const result = compareProps(
      { target_env: "prop.env", coverage_threshold: "prop.coverage" },
      [
  ...
  test("未知字段检测", () => {
    const result = compareProps(
      { bad_field: "prop.env" },
      [
        { name: "target_env", type: "string", required: true },
      ]
    );
...
describe("集成测试：scope → resolve → compareProps", () => {
  ...
  test("compareProps 验证 builtin part 的参数覆盖", () => {
    ...
    const result = compareProps(
      { feature_ref: "prop.branch", message: '"chore: update"' },
      [
