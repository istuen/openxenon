...
describe("TaskSandbox (Task 3.1)", () => {
  ...
  test("添加和删除 Part", () => {
    ...
    state.currentIR.expectations = [
      { name: "check_build", probeRef: "@oxn/probe/build", params: {}, errMsg: "fail" },
    ];
  ...
  test("本地优先原则合并全局 IR", () => {
    ...
    const state: SandboxState = {
      taskId: "test", sandboxDir: "/tmp", sandboxBlueprintPath: "/tmp/bp.json",
      originalBlueprintPath: "/tmp/bp.json", currentIR: sandboxIR,
    };
...
describe("MutationValidator (Task 3.2)", () => {
  ...
  test("abstract part 含 execution 被拦截", () => {
    ...
    mutated.abstractParts.push({
      name: "bad", isAbstract: true, implements: "test",
      props: [], probes: [], execution: ["probe.x"],  // 违规
    });
