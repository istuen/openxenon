...
function blueprintAstToIR(
  ast: MockBlueprint,
  concreteParts: MockPart[]
): OxnAssemblyIR {
...
function resolvePropValue(
  ...
  switch (paramValue.kind) {
    ...
    case "templateString": {
      return paramValue.value.replace(
        /\$\{prop\.(\w+)\}/g,
        (_: string, key: string) => String(props[key] ?? "")
      );
    }
    default:
      return undefined;
...
function assemblyPartToFrozenPart(
  part: OxnAssemblyConcretePart,
  ...
  for (const prop of part.props) {
    ...
    if (finalParams[prop.name] === undefined && prop.required) {
      throw new Error(
        `Part "${part.name}" 缺少必填参数 "${prop.name}"`
      );
    }
  }
  ...
  return {
    id: part.name,
    name: part.implements
      ? `${part.name} (implements ${part.implements})`
      : part.name,
    deps: [],
    params: finalParams,
    ...
    probes: part.probes.map((p) => {
      ...
      if (p.params) {
        for (const [key, expr] of Object.entries(p.params)) {
          resolvedProbeParams[key] = resolvePropValue(
            expr,
            finalParams,
            new Map()
          );
...
function assemblyToFrozenBlueprint(
...
function validatePropTypes(
  ...
  for (const prop of props) {
    ...
    if (prop.type === "number" && typeof value !== "number") {
      errors.push(
        `Prop "${prop.name}" 类型不匹配：期望 number，实际 ${typeof value}`
      );
    }

    if (prop.type === "boolean" && typeof value !== "boolean") {
      errors.push(
        `Prop "${prop.name}" 类型不匹配：期望 boolean，实际 ${typeof value}`
      );
    }
    ...
    if (prop.type.startsWith("enum")) {
      ...
      if (!allowed.includes(String(value))) {
        errors.push(
          `Prop "${prop.name}" 枚举越界：值 "${value}" 不在 [${allowed.join(", ")}] 中`
        );
      }
    }
  }
...
function validateRequiredParams(
  ...
  for (const prop of props) {
    if (
      ...
      taskProps[prop.name] === undefined
    ) {
      errors.push(
        `必填参数 "${prop.name}" 未在 Task binding 中注入`
      );
    }
  }
...
describe("Mock Pipeline: AST → OxnAssemblyIR → FrozenBlueprint", () => {
  ...
  test("模板字符串求值：${prop.xxx} 正确替换", () => {
    ...
    expect(probe.params.command).toBe(
  ...
  test("类型一致性校验：枚举越界报错", () => {
    const props: OxnAssemblyProp[] = [
      { name: "env", type: 'enum("dev", "staging", "prod")', required: false },
    ];
  ...
  test("全链路闭环：完整的 compiledBlueprint 包含所有必要字段", () => {
    ...
    // Step 1: 校验 binding
    const absResult = validateAbstractPartBindings(
      ir,
      taskBinding.partBindings
    );
    ...
    // Step 3: 校验覆盖率
    const covResult = validateRequiredParams(
      ir.props,
      taskBinding.propBindings
    );
