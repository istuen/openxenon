// E2E Task Definition — 独立 Task 解析验证
...
describe('E2E Task — 独立 Task 解析验证', () => {
  let taskIR: OxnAssemblyTaskIR
  ...
  beforeAll(async () => {
    ...
    const uri = URI.file('/virtual/test-task.oxn')
    const factory = shared.workspace.LangiumDocumentFactory
    const doc = await factory.fromString(TASK_OXN, uri, token)
    ...
    taskIR = categories.tasks[0]!
    expect(taskIR).toBeDefined()
    expect(taskIR.name).toBe('verify-e2e')
    expect(taskIR.use).toBe('@prj/blueprints/e2e-flow')
    slotBindings = taskIR.slotBindings
  })

  test('Task 解析成功，含 slot binding', () => {
