...
| `observe = ["ShellExec"]` | No | Probe names this slot observes (AI triggers via `oxn probe run` at runtime; submit only writes trace.jsonl, no longer writes frozen.json) |
