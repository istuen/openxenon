...
| **workflow** | 执行流程（slot DAG，原 Blueprint 改名） | Slots（仅 desc 字段）| env, timeout; build, test, verify |
| **stack** | 技术栈约束（runtime/linter/test） | Tools | typescript, biome, bun-test |
| **blueprint** | 组合模板（domain + workflow + stack 组合） | **Use** + **Boundaries**（跨类型引用）| integrate-payment, dev-standard |
...
> - **External 已移除**：外部资源统一通过 Asset Paper Schema 的 `references: [{ url }]` 字段声明（无 `## Externals`）
...
**H2 categories**：`## Use`（引用 3 边界）+ `## Boundaries`（编排单元）
