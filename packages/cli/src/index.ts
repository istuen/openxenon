// =============================================================================
// `oxn` CLI 入口 (v0.6 — packages/cli/ 阶段 5 迁移版)
//
...
//
// v0.6 阶段 5 迁移：从 src/cli/ 搬至 packages/cli/src/
//   - 所有子命令文件在 ./commands/ 子目录下
//   - 相对路径: './context' → './commands/context', '../daemon' → 仍指向 src/daemon (未迁)
//   - 'src/skills/' 引用在阶段 6 后改为 '@openxenon/cli/skills' alias
// =============================================================================
...
import { cliContext, detectCliFormat, detectVerbosity } from './commands/context'
...
const main = defineCommand({
  ...
  subCommands: {
    ...
    config: () => import('./commands/config-cmd').then((m) => m.default),
