/**
 * src/infra/oxnrc.ts (engine L1-Infra)
 *
 * .oxnrc loader — reads project-level .oxnrc JSON config.
 * Used by both L3-CLI and L3-Hall (Hall scans .oxnrc for project metadata).
 *
 * Resolution priority (highest first):
 *   1. CLI flag `--leader-mode=<reference|mvp>`  (handled in src/cli/index.ts)
 *   2. Environment variable `OXN_LEADER_MODE`
 *   3. Project config file `./.oxnrc`           (auto-loaded by this function)
 *   4. Default: `reference`
 *
 ...
 * the resolution chain continues with env / default. The error is surfaced
 * via the `warning` string so callers can decide whether to log it.
 */
