...
| T11 three-layer PR-1 grammar | `feat/v0.2-t11-three-layer-pr1-grammar` | W5c | **✅ done** (commit aaad25d, merge 6e3000f) — InvariantDecl 加 script/manual/scope 可选字段 + WorkDeclaration 加 domainProofs+=DomProofRef (`proofs [...]` 语法) + 重新生成 parser/ast/grammar + 2 处兼容修复 (domain.ts + oxn-generator.ts) + 10 case 新测试 (6 invariant-decl + 4 work-domain-proofs) + ⚠ 串行约束: T11 在 T10 之后合入, 第二次 langium:generate 不出冲突 |
