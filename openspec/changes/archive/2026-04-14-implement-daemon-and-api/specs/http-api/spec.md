...
- Scans global proofs at `~/.xenonix/proofs/`
- Scans project proofs at `<project>/.xenonix/proofs/`
- Returns merged list with project proofs taking priority

#### Scenario: List proofs
- **WHEN** GET request to `/api/v1/proofs/list` with `X-Project-Path`
- **THEN** system returns `{"proofs": [{"id": "...", "name": "...", "type": "...", "path": "..."}]}`
...
- **THEN** system creates task and returns `{"taskId": "...", "status": "pending"}`
