# 术语双层 SSOT 架构 RFC（Domain 内部 + glossary 外部）

> **日期**：2026-08-01（v0.1 Draft）
> **状态**：⚠ **Superseded by [RFC-0017-terminology-two-tier-ssot](../../../docs/rfc/zh-cn/RFC-0017-terminology-two-tier-ssot.md)**（2026-08-02 归档）
> **归档原因**：本草案经 2026-08-01 `/grilling` session 3 轮质询后已收敛为 6 项冻结决议 + 5 项 loose ends（详见 `.openxenon/drafts/terminology-ssot-execution-plan.md`），并 promote 到 `docs/rfc/zh-cn/RFC-0017-terminology-two-tier-ssot.md` 作为正式 RFC。保留本文件作历史溯源。
> **基础**：[RFC-0009 文档三情态分离](../rfcs/zh-cn/RFC-0009-doc-three-modalities.md) · [RFC-0014 Asset 注入机制](../rfcs/zh-cn/RFC-0014-asset-injection-mechanism.md) · [RFC-0018 项目工程元层与 SSOT 全景](../rfcs/zh-cn/RFC-0018-project-engineering-meta.md) · [ADR-0059 Domain reference model v2](../adrs/0059-domain-reference-model-v2.md)
> **作者**：opencode（与 user 协作，2026-08-01）
> **范围**：术语体系角色分工（Asset Domain vs glossary）；glossary 物理形态由多文件收为单页；8 个 `docs/product/zh-cn/concepts/*.md` 概念页去重定义
> **关联 RFC**：RFC-0009 / RFC-0014 / RFC-0018 / ADR-0059
> **目标版本**：v0.7+（具体批次待定）
...
| 层 | 物理位置 | 体裁 | 行数 |
|---|---|---|---|
| **Asset Domain** | `.openxenon/assets/domains/*.md`（9 文件） | 内部定义（含 desc + inv + ban + 来源标注） | 1429 |
| **glossary** | `docs/glossary/zh-cn/*.md`（9 文件） | Doc 镜像（含 desc + `source:` frontmatter） | 678 |

二者关系约定上是 "Domain → glossary 单向镜像"，但**实际有 3 个设计缺陷**：

1. **角色错位**：glossary 在物理上属于 Doc 层，但功能上承担"对外术语词典"——这是 RFC-0009 D5 警告的"两情态组合是设计错误信号"（Asset 视角的"desc" + Doc 视角的"对外手册"混在同一文件）
2. **同步缺失**：约定单向同步但**无自动化**——glossary `synced-at: 2026-07-22` 与 Domain `synced-at: 2026-07-31` 已漂移 9 天
3. **形态分散**：9 个 glossary 文件按"主题分类"切分（asset-terms, cli-terms, ...），但外部用户需要的是**按字母查词**的字典，不是按内部主题切分的镜像

### 0.2 当前 8 个概念页与 glossary 的内容重复

`docs/product/zh-cn/concepts/` 已有 8 个概念页（`asset.md`、`iap-paradigm.md`、`work.md`、`proof.md`、`insight.md`、`lifecycle.md`、`asset-paper.md`、`_index.md`），每个都含 term 定义段。这与 glossary 形成**重复定义**：

- glossary 定义 `Asset`（asset-terms.md）
- `concepts/asset.md` 又定义 `Asset`
- Domain（oxn-asset-domain.md）又定义 `Asset`

同一个 term 在 3 处出现，3 处都可能是 stale——读者无所适从。

### 0.3 设计意图的重新对齐

2026-08-01 `/grilling` session 重新对齐：

> "glossary 应该作为整个 MD 的术语词典，其他文档都使用这里的术语作为标准。"
> —— user, 2026-08-01

这句话把当前"glossary 是 Domain 镜像"的弱关系，提升为**glossary 作为外部 SSOT**——但**不动内部 SSOT**（Asset Domain 仍是术语权威定义源）。新设计：

- **Asset Domain = 内部 SSOT**（OpenXenon Runtime / Engine / AI Agent 视角）
- **glossary = 外部 SSOT**（用户 / 文档读者视角）= Domain 在 Doc 层的聚合与镜像

术语的新增/编辑/删除只能从 Domain 入口进入，glossary 通过单向 sync 脚本同步。
...
合并去重规则：
  ...
  d. 已知 16 个碰撞 term 需逐个验证 root 归属（详见执行计划 §6）：
...
> D7 root 解析基于 v0.6 扁平 9-Domain 结构；[v0.7 层级 RFC](../v0.7-domain-hierarchy-restructure-rfc.md)（`.openxenon/drafts/rfc/v0.7-domain-hierarchy-restructure-rfc.md`）落地后需重定向 root 解析逻辑（按新的 Layer 0/1/2 层级判定 root）。
...
### Phase 1：RFC promote（v0.7.x 批次）

1. 本 RFC 拍板（Draft → Accepted）
2. 编号分配（建议 **RFC-0017**，接 RFC-0016 之后；RFC-0018 已占）
...
- [RFC-0009 文档三情态分离](../rfcs/zh-cn/RFC-0009-doc-three-modalities.md) — Asset/RFC/Doc 三情态基线；D3 边界规则是本 RFC 合规依据
- [RFC-0014 Asset 注入机制](../rfcs/zh-cn/RFC-0014-asset-injection-mechanism.md) — Domain 解析
- [RFC-0018 项目工程元层与 SSOT 全景](../rfcs/zh-cn/RFC-0018-project-engineering-meta.md) — Meta 层 5 类 + 跨层引用规则
...
- 待 review 项：
  ...
  - Phase 4 删除 `docs/glossary/` 时 en/ stub 处理路径
