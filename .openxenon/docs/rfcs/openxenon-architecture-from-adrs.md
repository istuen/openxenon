...
| **确定性验证** | Kernel + Proof | OXN 出示真实验证结果 | 0008, 0031 |
...
| 实体 | 性质 | 主导权 | 对应代码模块 |
|---|---|---|---|
| **E1 Asset** | 静态硬约束边界 | 工程师 | `packages/engine/src/Asset/` |
| **E2 Work** | 动态协作（IAP 三阶段 + Round） | 工程师 ↔ AI | `Intent/` + `Align/` + `Proof/` |
| **E3 Engine** | 独立验证主权基座 | OXN | L0-L2 全部 |
| **E4 Insight** | 涌现层（1+1>2） | AI 推理 | `packages/engine/src/Insight/` |
...
**kind-isolation 原则**（2026-07-10 journal）：
...
| Task 沙箱 + @ 命名空间 | 0025 | Task.oxn 可含本地 Part/Probe 定义；命名纪律：./ @oxn/ @prj/ @gbl/ |
...
| probe-stats 跨 proof 累积 | probe-stats.json 持久化，跨 proof 累积行为特征 |
| 行为特征观测 | AI 用了哪些边界 / 触碰了什么 / Loop 收敛速度 |
...
## 3. AI 协作哲学（ADR-0012 / ADR-0022 / ADR-0032）
...
### 3.2 三模式 AI 执行（ADR-0022 · Partial）

| 模式 | 说明 | 状态 |
|---|---|---|
| **Guided** | AI 在 Asset 边界内自由发挥；Engine 审计 | ✅ v0.6.1 默认 |
| **Adaptive** | AI 可根据反馈调整策略 | 🟡 实验（Hall only） |
| **Unmanaged** | AI 完全自主 | 💡 提案 |

### 3.3 信息隐藏（ADR-0012 + ADR-0058）
...
| 2 | **Kernel 真空**：零 IO、零 Langium、零 fs/net/child_process | 0008, 0010, 0013 | L0 |
| 3 | **证据链顺序**：Trace-before-State（trace 先于 state 写入） | 0009, 0024 | L0-L2 |
| 4 | **证据链三件套**：frozen.json + trace.jsonl + state.json | 0011 | L0-L2 |
| 5 | **审计链**：AI 被 trace 约束，非预防限制 | 0012, 0031 | E2-E3 |
| 6 | **@ 寻址纪律**：统一 @ 命名空间，-> 伪指针否决 | 0023 | L1 |
| 7 | **catalog.json + Probe 排除**：机器解析用 catalog，AI 不可见 Probe | 0035 | E1-L3 |
| 8 | **三边界正交**：Domain/Workflow/Stack 互不引用 | 0054 | E1 |
| 9 | **Blueprint 隔离**：唯一跨类型组合实体 | 0055 | E1 |
| 10 | **四层确定性**：边界/验证/证据/记录 | 0057, 0058 | 全局 |
...
## 5. Supersession Chain（ADR 废弃链）

```
ADR-0019 (Blueprint Type) ──────→ ADR-0054/0055 (三边界框架)
ADR-0033 (IAP 命名) ──────────→ v0.6.1 命名统一
ADR-0034 (Work vs Daemon) ────→ v0.6 Daemon 重构
ADR-0036 (canonical.oxn) ─────→ blueprint.oxn + frozen.json 三件套
ADR-0040-0047 (Memory 系列) ──→ ADR-0048-0051 (Asset Paper) + ADR-0049 (context.md)
ADR-0048 (library/external) ──→ ADR-0053 记录废弃 + ADR-0056 接管 External
```
...
| **v0.7** | 信任链效率层 | 0007, 0022, 0028 (Daemon + Insight + Token + Hook) |
...
- [版本统一 RFC](../rfcs/version-unification-rfc.md) — 信任链叙事 + v0.6.1 最小信任闭环
