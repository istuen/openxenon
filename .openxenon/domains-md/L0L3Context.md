...
- items: L2Domain, Tier, TierLevel, Stratum, Strata, ArsenalPromote, DraftAsset, CanonicalAsset
...
- desc: L0 Kernel 不得 import 任何 L1+ 模块（src/kernel/ 内不出现 oxl/infra/builtin/work/cli/daemon/hall/skills/watcher/core/i18n 引用）

### inv-2
- value: L1 Foundation 不得 import L2/L3 模块
- desc: L1 Foundation 不得 import L2/L3 模块

### inv-3
- value: L2 Module 不得 import L3 Runtime 模块
- desc: L2 Module 不得 import L3 Runtime 模块

### inv-4
- value: L3 Runtime 允许 import 所有下层（入口层的特权）
- desc: L3 Runtime 允许 import 所有下层（入口层的特权）

### inv-5
- value: L0 Kernel 不得使用 fs / path / crypto / http / child_process / os / net 等平台 IO 模块
- desc: L0 Kernel 不得使用 fs / path / crypto / http / child_process / os / net 等平台 IO 模块

### inv-6
- value: L0 Kernel 不得访问 process.env / process.stdout / process.stdin
- desc: L0 Kernel 不得访问 process.env / process.stdout / process.stdin

### inv-7
- value: L0 Kernel 不得使用 EventEmitter 或维护运行时状态
- desc: L0 Kernel 不得使用 EventEmitter 或维护运行时状态

### inv-8
- value: L2 层命名为 Module 而非 Domain（避免与 OXL 的 Domain 实体产生歧义，参见 ADR-0006）
- desc: L2 层命名为 Module 而非 Domain（避免与 OXL 的 Domain 实体产生歧义，参见 ADR-0006）

### inv-9
- value: L0-L3 与 P0-P3 是两个正交概念：L0-L3 描述代码分层，P0-P3 描述产品路线图
- desc: L0-L3 与 P0-P3 是两个正交概念：L0-L3 描述代码分层，P0-P3 描述产品路线图

### inv-10
- value: Domain 实体（用户业务域）与 L0-L3 宪法（OXN 元域）通过 context_map 显式连接，不可互相包含
- desc: Domain 实体（用户业务域）与 L0-L3 宪法（OXN 元域）通过 context_map 显式连接，不可互相包含

### inv-11
- value: OXN 元域的宪法文档（l0-l3-constitution.md）不在用户业务域的文档体系内
- desc: OXN 元域的宪法文档（l0-l3-constitution.md）不在用户业务域的文档体系内
