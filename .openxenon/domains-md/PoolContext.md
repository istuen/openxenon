...
### PoolEntry
- name: PoolEntry
- desc: 单个池条目: pool/slug/title/content/metadata, 物理路径 .openxenon/pools/<pool>/<slug>.md
...
### HeadingSkeleton
- name: HeadingSkeleton
- desc: 每个 .md 文件的 heading 骨架校验: 行首 ^#{1,6}\s, 排除 ```代码块``` 内误识别
...
### CalibrationSignal
- name: CalibrationSignal
- desc: work finalize 阶段写入 pools/research/<slug>/frozen.json 的校准信号 (T12 集成)

### ForgeDeprecation
- name: ForgeDeprecation
...
- items: Forge, DesignNote, WorkLog, ResearchPaper
...
- desc: 5 池 heading 模板互为独立 spec, 不可交叉 (如 research 池不可出现 '# 决策记录')

### inv-2
- value: pool-writer.ts slug 必须匹配 /^[a-z0-9][a-z0-9-]*$/ (小写中划线)
- desc: pool-writer.ts slug 必须匹配 /^[a-z0-9][a-z0-9-]*$/ (小写中划线)

### inv-3
- value: check-heading-skeleton.ts 仅检查 pools/ + forges/ (不影响其他 .md)
- desc: check-heading-skeleton.ts 仅检查 pools/ + forges/ (不影响其他 .md)

### inv-4
- value: forges/ 兼容期: warnOnForgesDeprecated 默认 false (静默), Sprint 6 flip 后打印 WARN
- desc: forges/ 兼容期: warnOnForgesDeprecated 默认 false (静默), Sprint 6 flip 后打印 WARN

### inv-5
- value: pool 条目 frozen.json chmod 0o444 + content_hash 不可篡改 (复用 writeFrozenImmutable)
- desc: pool 条目 frozen.json chmod 0o444 + content_hash 不可篡改 (复用 writeFrozenImmutable)
