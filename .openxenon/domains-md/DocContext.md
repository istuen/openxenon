...
- items: Arsenal, Core Engine, glo, Jekyll, MkDocs, Docusaurus, refs, handle, ref-logic
...
- desc: 站点根路径 / 必须可访问；不在根路径放 source markdown

### inv-2
- value: i18n 必须用对称 prefix 目录（zh-cn/ 与 en/），禁用 root + prefix 混合结构
- desc: i18n 必须用对称 prefix 目录（zh-cn/ 与 en/），禁用 root + prefix 混合结构

### inv-3
- value: Sidebar 6 桶固定：开始 / 范式与核心概念 / IAP 三轴 / 实战 / 参考 / 附录
- desc: Sidebar 6 桶固定：开始 / 范式与核心概念 / IAP 三轴 / 实战 / 参考 / 附录

### inv-4
- value: Sidebar 章节文案用中文显示，slug 保持英文（IAP 术语锁定）
- desc: Sidebar 章节文案用中文显示，slug 保持英文（IAP 术语锁定）

### inv-5
- value: URL 必须含 .html 后缀（GitHub Pages 项目页不支持 cleanUrls）
- desc: URL 必须含 .html 后缀（GitHub Pages 项目页不支持 cleanUrls）

### inv-6
- value: 对外发布只用 dev 分支推送触发 Pages（main 留作 production 槽位）
- desc: 对外发布只用 dev 分支推送触发 Pages（main 留作 production 槽位）

### inv-7
- value: Pages 部署工作流 docs.yml 的 deploy 守卫仅允许 dev 分支 push 或 workflow_dispatch
- desc: Pages 部署工作流 docs.yml 的 deploy 守卫仅允许 dev 分支 push 或 workflow_dispatch

### inv-8
- value: feature/* 分支 push 触发 build 验证但不部署
- desc: feature/* 分支 push 触发 build 验证但不部署

### inv-9
- value: 对外 SSOT 是 docs/zh-cn/*.md（v0.1.0 中文为首发）
- desc: 对外 SSOT 是 docs/zh-cn/*.md（v0.1.0 中文为首发）

### inv-10
- value: 旧 docs/{core,architecture,reference,guides,design,horizon,changelog} 备份到 _archive/，不再作对外引用源
- desc: 旧 docs/{core,architecture,reference,guides,design,horizon,changelog} 备份到 _archive/，不再作对外引用源

### inv-11
- value: 工程设计沉淀 .openxenon/forges/ v0.1.0 后从 git 移除（不对外）
- desc: 工程设计沉淀 .openxenon/forges/ v0.1.0 后从 git 移除（不对外）
