...
branch: feat/goal-infra-ports
source: draft
source-ref: .openxenon/drafts/.archived/rfc/v0.7.0-infra-ports-rfc.md
note: |
  从 dev/versions/infra-ports（已退役）回滚。D5+ 起前瞻 intent 留在 Goal 层（dev/pool/）；cut 时由 `oxn version cut` 收录到 Version 文档。Source Draft 见 source-ref。
rfc:
  - dev/pool/emergence.md
adr:
